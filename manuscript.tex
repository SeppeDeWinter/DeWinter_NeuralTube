...
and nucleotide importance scores reveal TFBS for a SOX monomer, a SOX dimer and TFAP2 (\textbf{Fig. 2i}). 
...
grantFIMOScanningOccurrences2011, schreiberTangermemeBiologicalSequence2025
} compared to FOXA2 target enhancers that are uniquely accessible in the floorplate cells (\textbf{Fig. S4b}). 
...
independently of the cell type (i.e., indepdent of cooperating TFS), but mainly to regions with multiple sites, and 
high-affinity sites.
...
and the presence of instances of the \textit{CATYMATCA} motif correlates with the expression of \textit{MEIS1/2}, \textit{PBX1/2} and \textit{HOXB3} (\textbf{Fig. 4d-e} and Fig. S5). 
...
topic (\textbf{Fig. 4d-e} and Fig. S5)\par
...
The importance of TEAD instances correlates with \textit{TEAD3/4} expression, GRHL with \textit{GRHL1/1} 
...
we asked whether $DeepNeuralTube$ could explain the effect of some of these variants on enhancer activity. For all suggestive SNPs we scored both the reference and alternative allele(s) using both $DeepNeuralTube^o$ and $DeepNeuralTube^e$ 
...
Most of the "explainable" SNPs have a high delta prediction score for either organoid neural crest Topic 58 or Topic 59 (\textbf{Fig. S7}). 
...
(\textbf{Fig. 6b-c}), correlating with the expression of \textit{GATA2} (\textbf{Fig. 6d} and Fig. S8).\par
...
but also provide counter exampled to this paradigm. 
...
$DeepNeuralTube_O$ and $DeepNeural$- $Tube_E$ are available via Zenodo (\href{https://doi.org/10.5281/zenodo.15280773}{10.5281/zenodo.15280773}). 
...
\begin{itemize}
  \item \href{https://genome.ucsc.edu/s/Seppe%20De%20Winter/neural%20tube%20doro%2Dventral}{dorsal-ventral UCSC session}
  \item \href{https://genome.ucsc.edu/s/Seppe%20De%20Winter/neural%20tube%20neural%20crest}{neural crest UCSC session}
  \item \href{https://genome.ucsc.edu/s/Seppe%20De%20Winter/neural%20tube%20neuron}{neuron UCSC session}
...
We also thank Prof. Dr. Tatjana Sauka-Spengler and Dr. Ruth Williams for their great support with helping setting up the chicken embryo enhancer reporter assays.
Computing was performed at the Vlaamse Supercomputer Center and VIB datacore. 
This work was funded by the following grants to 
S. Aerts , %TODO: Add funding
PhD fellowship from FWO to S. De Winter (1191323N) and 
Post doctoral fellowship from ... to C. Mannens (), %TODO: add funding 
