...
\affil[5]{Karolinska Institute, Division of Neurodegeneration, Department of Neurobiology, Care Sciences and Society,Stockholm, Sweden}
...
%need
Enhancer code studies are predominantly limited to a small number of cell types often focusing
on model organisms.
%task and results
...
Enhancers encoding anterior-posterior patterning form an exception to this rule where they are encoded using
homotypic clusters of homeodomain binding sites.
%conclusion
This compendium of enhancer codes thus provides gene regulatory insight into human development.
%implication
...
Central to the study of the genomic code is the paradigm that 
...
In both systems we could identify neuronal progenitors, early differentiating neurons,
...
Next, we sub-clustered the neuronal progenitor, early neuron, and neural crest cells (\textbf{Fig. 1d-f}). 
...
and identified equivalent cells in the embryo where these same regions were also co-accessible.
...
\begin{figure}[b!]
  ...
  \caption{
    (Continued)
    \textbf{d-f}, UMAP based on gene expression profiles of organoid (top) and embryo (bottom) cells types showing sub-clusters of neuronal progenitors (\textbf{d}), early differentiating neurons (\textbf{e}) and neural crest (\textbf{f}) along with expression of indicated genes in red-green-blue color scale.
...
In order to gain insight into the sequence characteristics underlying cell type-specific chromatin accessibility of 
neuronal progenitor, neural crest and neuronal cell type-specific enhancers, 
...
\begin{figure}[p]
  ...
  \caption{
    ...
    \textbf{a}, UMAP of organoid (top) and embryo (bottom) neuronal progenitors colored by cell-topic probabilities in RGB color scales of dorso-ventral topics. 
    (Continues next page).
  }
\end{figure}
\afterpage{\clearpage}
...
\begin{figure}[b!]
  ...
  \caption{
    ...
    Two-sided p-value calculated using Mann-Whitney U test between distribution of number of FOX instances in general (n = 236) and specific (n = 3114) regions, for organoid.
    \textbf{k}, Boxplot showing $-log_{10}(p value)$ of FOXA2 motif hits calculated using FIMO\cite{grantFIMOScanningOccurrences2011,schreiberTangermemeBiologicalSequence2025} for general regions (bottom) and specific regions (top) stratified by the number of FOXA2 instance per region. 
    Two-sided p-values calculated using Mann-Whitney U test between distribution of motif scores for 
    general regions with one FOX instance ($n_{organoid} = 47$ and $n_{embryo} = 42$) 
    and specific regions with one FOX instance ($n_{organoid} = 1,508$ and $n_{embryo} = 1,467$).
    \textbf{l}, \textit{de novo} motif for regions with a single FOXA2 instance that are specific (left, top) or general (right, top) and heatmap color coded by nucleotide identity for individual regions for both specific (left, bottom) and general (right, bottom) regions that have a single FOXA2 instance.
  }
\end{figure}


\subsection*{
  Homotypic clusters of distinct homeodomain transcription factor binding sites encodes anterior-posterior patterning
}
...
\begin{figure}[p]
  ...
  \caption{
    \textbf{An unsophisticated enhancer code underlies anterior-posterior patterning in neuronal progenitors.} \\
...
instances of patterns from neuronal progenitors are important (\textbf{Fig. 6b-c}).
...
\subsection* {
  Four to five distinct transcription factor binding sites in a single nucleosome depleted region are sufficient to encode
  all neuronal progenitor, neuronal and neural crest cell states.
}
...
Notably, all models for neural crest related states performed best (\textbf{Fig. 7a}).\par
...
AP-patterning enhancers seem to be an exception to this rule, where they are encoded using multiple instances of only two and sometimes even a single TFBS (\textbf{Fig. 7a}).
...
with instances with 4 bp spacing being the most accessible (\textbf{Supplementary Fig. 8}). 
