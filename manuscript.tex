...
Genomic enhancer code studies have been limited to specific cell types in model organisms. 
We created a compendium of enhancer codes for all cell types of the human neural tube and neural crest. 
We generated a single-cell multiome atlas of human neural tube organoids and of the head of a four-week post conception 
human embryo. Using DNA sequence based deep learning - DeepNeuralTube - we identify 15 cell types that are shared 
between the organoid and the human embryo and comprehensively analyse the enhancer code of each individual cell type. 
We confirm the paradigm that enhancers are encoded as heterotypic clusters of 4-5 TF binding sites within one 
nucleosome depleted region, using soft syntactic rules. We also provide counter examples to this rule, with enhancers 
encoded as homotypic clusters and by dimer motifs. Finally, we validate DeepNeuralTube using enhancer reporter assays 
and by interpreting genetic variation associated with facial features. 
...
in direct comparison to an enhancer compendium we generate in parallel for the head of a four-week post conception human 
embryo. Through these comparisons we assess the enhancer code paradigm and construct an enhancer annotation and code 
resource that is a consensus between the organoid model and the human embryo.\par
...
Note that similar transcriptomic states were obtained from all three cell lines (\textbf{Fig. S1a-b}) demonstrating the robustness of the organoid system.\par
...
Both organoid and embryo neural progenitors show a gradient of different cell states reflecting dorsal-ventral patterning, evident from the expression of dorsal-ventral marker TFs\cite{delasRepressiveInteractionsGene2020}:
...
"$DeepNeuralTube^o$" and "$Deep$-$NeuralTube^e$", on the organoid and embryo atlases respectively. 
...
\textit{in situ} hybridization, and given that it is difficult to visually distinguish between the neural tube and pre-
migratory neural crest, this result is not surprising.\par
...
Focusing on neural progenitors, we identified topics representing cells and regions of different dorsal-ventral
...
To investigate the enhancer code underlying dorsal-ventral patterning of neural progenitors further, we clustered 
...
resulting in a dorsal-ventral enhancer code-table (\textbf{Fig. 3f}).
Both $DeepNeuralTube^o$ and $DeepNeuralTube^e$ found similar patterns underlying the dorsal-ventral states and the 
...
are important (\textbf{Fig. 3f-g}) and the importance of these instances correlates with the expression of respectively \textit{FOXA1/A2/P2/P4}, \textit{TEAD1} and \textit{RFX3/4} in both the organoid and embryo (\textbf{Fig. 3h} and Fig. S3). 
...
To assess the ability of DeepNeuralTube for prioritizing enhancers with cell type-specific activity we selected a genomic
region that was predicted to be specific for ventral neural progenitors (\textbf{Fig. 3k}). Explaining this region,
...
In the FOXA2 target enhancers of the floorplate that are also bound by FOXA2 in other systems (HepG2 and A549 cell lines), 
we identified a higher number of FOXA2 sites using FIMO\cite{
grantFIMOScanningOccurrences2011, schreiberTangermemeBiologicalSequence2025} compared to FOXA2 target enhancers that are uniquely accessible in the floorplate cells (\textbf{Fig. S4b}). 
...
\subsection*{
  Distinct homeodomain transcription factor binding sites encode anterior-posterior patterning
}
...
and the presence of instances of the \textit{CATYMATCA} motif correlates with the expression of \textit{MEIS1/2}, \textit{PBX1/2} and \textit{HOXB3} (\textbf{Fig. 4d-e} and Fig. S5), which resembles cooperative binding of Hox proteins 
...
topic (\textbf{Fig. 4d-e} and Fig. S5). The binding specificity of these TFs might be modulated by another co-factor, 
...
and AP2 with both \textit{TFAP2A} and \textit{TFAP2C} (\textbf{Fig. 5d} and Fig. S6). 
Note that ZEB instances have a positive contribution to the pre-migratory while they have a
...
The positive contribution of ZEB in the pre-migratory neural crest state coincides with the absence of the expression of both \textit{ZEB1} and \textit{ZEB2} (\textbf{Fig. 5d} and Fig. S6). 
...
the SOX-dimer with expression of \textit{SOX10} (\textbf{Fig. 5d} and Fig. S6). 
...
for which instances of RFX are important (\textbf{Fig. 5b-c}) correlating with the expression of \textit{RFX3/4} (\textbf{Fig. 5d} and Fig. S6).\par
...
that of the dimer motif with the expression of both \textit{TWIST1} and \textit{ALX1/4} and \textit{PRRX1} (\textbf{Fig. 5d} and Fig. S6). 
...
We scored both the reference and alternative allele(s) using both $DeepNeuralTube$ models and calculated the difference 
in prediction score between alleles (delta prediction score). 
At an absolute delta prediction score threshold of 0.2 we identified 235 SNPs, out a total of 23,701 SNPs associated 
with facial variation, that are explainable by either of the two models (\textbf{Fig. 5f}). 
Most of the "explainable" SNPs have a high delta prediction score for either the migratory neural crest class 
(organoid Topic 59) or the facial mesenchyme class (organoid Topic 58; \textbf{Fig. S7}).
...
For the cells representing early neuronal differentiation, we identified several topics for different stages of neuronal fate
acquisition in both organoid and embryo (\textbf{Fig. 6a}). Again, we made use of $DeepNeuralTube^o$ and
...
\textit{FOXA2/J3/O1/P1/P2/P4} and \textit{RFX3/4} (\textbf{Fig. 6d} and Fig. S8). 
Instances of a SOX monomer motif is important for one of these topics (\textbf{Fig. 6b-c}), 
correlating with the expression of \textit{SOX2} (\textbf{Fig. 6d} and Fig. S8). 
...
and correlated with the expression of \textit{ISL1} (\textbf{Fig. 6d} and Fig. S8). 
Finally, in the organoids we could identify an additional topic with instances of GATA
(\textbf{Fig. 6b-c}), correlating with the expression of \textit{GATA2} (\textbf{Fig. 6d} and Fig. S8), a marker 
...
To get insight into the minimal set of TFBS that are needed to encode these cell states we trained
...
However, the genome wide precision and recall of these models to identify topic regions is still low (Fig. S9). \par
...
These results indicate that enhancers specific to DV-patterning, neural crest states and early neuronal differentiation are encoded in the human genome using around four to five distinct TFBS (\textbf{Fig. 7a}) 
...
coefficient for each logistic regression model along with the number of each patternsused for enhancer design
represented by the size of each dot (middle)
(Continues next page)}
\end{figure}
\afterpage{\clearpage}
...
\begin{figure}[t]
  ...
  \caption{
  (Continued) 
   and boxplots (n = 200) showing prediction score of designed enhancers for each topic (bottom). Cell type-classes are indicated by colors. 
...
effectively be reproduced using human neural tube organoids both at transcriptomic level and at the level of 
enhancer codes. Specifically, we identified a total of fifteen distinct cell states with one-to-one matches between 
the organoid and the embryo. The fact that cell states were defined in a data driven manner by making use of topic 
modeling and that each of these states were independently identified in two separate datasets increases the confidence 
of their biological relevance. Furthermore, this high similarity between organoid and embyro derived cell 
...
whereby, instances with a 4 bp spacer were most accessible (\textbf{Fig. 10}). 
...
Accordingly, we observed that enhancers that are specific to the floorplate contain either less FOXA2 binding sites 
...
Our results indicate that enhancers underlying neural progenitor AP-patterning form an exception to the heterotypic rule 
of 4-5 different TFBS per enhancer. 
Homotypic clusters of 3-4 homeobox instances were sufficient to (\textit{in silico}) design enhancers specific to a certain AP domain. 
...
This observation is similar to Hox-Exd co-binding, where sequence specificity of HOX proteins is modulated by another 
co-factor\cite{slatteryCofactorBindingEvokes2011}.
...
We confirm the paradigm that most human enhancers consist of heterotypic clusters of TFBS with soft syntactic rules 
but also provide counter example to this paradigm. 
Furthermore, we validate the biological relevance of neural tube organoids, 
having representative transcriptomes and genomic regulatory code activation compared to embryo development. 
...
Alongside these models, we implemented new tools in our genomic deep learning framework CREsted\cite{kempynckCREstedModelingGenomic2025}, specifically for the analysis of compendia of enhancer codes underlying complex biological systems.
...
In this study we focused on predicting TF binding to enhancers, without much attention to their effector function.
The effect of a TF on target gene expression depends on its effector domain and interactions with
co-factors\cite{lambertHumanTranscriptionFactors2018} and this may also depend on the sequence context of
...
For the neural tube, it is known that repressive interactions in its core GRN are important\cite{delasRepressiveInteractionsGene2020}. 
In this study we could predict a strong repressive effect of ZEB1/2, a chromatin closing repressor that is used across a variety of biological
systems\cite{minnoyeCrossspeciesAnalysisEnhancer2020, taskiranCelltypedirectedDesignSynthetic2024}
(\textbf{Fig. S11}),
but we anticipate that additional repressor TFs can be added to gene-regulatory models with improved experimental and/or computational techniques in the future.
\par

Furthermore, the expression of a gene is generally regulated by multiple enhancers in the gene's locus\cite{
...
In this study we focused on the enhancer code of individual enhancers without addressing 
how the signal of multiple enhancers is integrated to control target gene expression. 
...
To conclude, we suspect that future models will take into account both the effector function of TFs through
...
Raw sequencing data of the human embryo will be made available through the European Genome Phenome Archive. %TODO: Add link to EGA
...
Data can be interactively explored using the \href{https://scope.aertslab.org/#/Neural_Tube/*/welcome}{neural tube SCope session} and on the UCSC genome browser: 
\begin{itemize}
  \item \href{https://genome.ucsc.edu/s/Seppe%20De%20Winter/neural%20tube%20doro%2Dventral}{dorsal-ventral UCSC
    session}: https://genome.ucsc.edu/s/Seppe\%20De\%20Winter/neural- \%20tube\%20doro\%2Dventral
  \item \href{https://genome.ucsc.edu/s/Seppe%20De%20Winter/neural%20tube%20neural%20crest}{neural crest UCSC session}: https://genome.ucsc.edu/s/Seppe\%20De\%20Winter/neural- \\\%20tube\%20neural\%20crest
  \item \href{https://genome.ucsc.edu/s/Seppe%20De%20Winter/neural%20tube%20neuron}{neuron UCSC session}: https://genome.ucsc.edu/s/Seppe\%20De\%20Winter/neural\%20- tube\%20neuron
  \item \href{https://genome.ucsc.edu/s/Seppe%20De%20Winter/neural%20tube%20anterior%2Dposterior}{anterior-posterior
      UCSC session}: https://genome.ucsc.edu/s/Seppe\%20De\%20Winter/neur- al\%20tube\%20anterior\%2Dposterior
\end{itemize}
...
This work was funded by the following grants to S. Aerts: ERCAdG (101054387); CZI (DI2-0000000068); SBO (S005024N); FWO 
(G0I2722N EOS ID 40007513, G094121N, G044124N); and Foundation Against Cancer (2020-1396 \& 2024-140), 
the following grants to S. Linnarson: Erling-Persson Foundation Human Developmental Cell Atlas grant, Knut and Alice
Wallenberg Foundation grants 2015.0041, 2018.0172 and 2018.0220, Swedish Foundation for Strategic Research SB16-0065, 
and EU Horizon2020 BRAINTIME project 874606 and 
PhD fellowship from FWO to S. De Winter (1191323N).
