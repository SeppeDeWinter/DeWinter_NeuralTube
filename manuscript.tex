...
We provide tools for building enhancer code compendia through our genomic deep learning package, CREsted.
...
From ventral to dorsal this is topic 33, 38, 36, 54 and 48 for the organoids and
...
Similarly, ZEB instances have a negative importance to the accessibility of regions in dorso-ventral domains except the most ventral one (\textbf{Fig. 3f} and Fig. S3). 
...
specific FOXA2 target regions have fewer FOX instances compared to general FOXA2 target regions (\textbf{Fig. S4b}). 
Furthermore, individual FOX instances in general FOXA2 target regions have higher affinity compared to FOX instances in
specific regions (\textbf{Fig. S4c}). These results suggest that the overall affinity of general FOXA2 target regions 
...
and the prescence of instances of the \textit{CATYMATCA} motif correlates with the expression of \textit{MEIS1/2}, \textit{PBX1/2} and \textit{HOXB3} (\textbf{Fig. 4d-e} and Fig. S5). 
...
anterior-posterior topics. 
  ...
  U.: UMAP. AP: anterior-posterior, DV: dorso-ventral, iter: iteration. 
  Also see Figure S5.
}
\end{figure}
\afterpage{\clearpage}
...
For the cells representing early neural differentiation, we identified several topics for different stages of neural fate
...
with instances with 4 bp spacing being the most accessible (\textbf{Fig. 10}). 
...
Homotypic clusters of 3-4 homeobox instances were sufficient to design enhancers specific to a certain AP domain. 
...
Alongside with these models, we implemented new tools in our genomic deep learning framework CREsted\cite{kempynckCREstedModelingGenomic2025}, specifically for the analysis of enhancer codes underlying complex biological systems.
...
\begin{itemize}
  \item \href{https://genome.ucsc.edu/s/Seppe%20De%20Winter/neural%20tube%20doro%2Dventral}{dorso-ventral UCSC session}
...
\begin{figure}[p]
  ...
  \caption{
    ...
    \textbf{a-b}, precision recall curves for classifying neural tube enhancers (left) and facial mesenchyme enhancers (right) using organoid (\textbf{a}) and embryo models (\textbf{b}). \textbf{c}, Chromatin accessibility profile of dorso-ventral topics for organoid
